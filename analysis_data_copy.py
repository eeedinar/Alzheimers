...
class Data_Analysis():
    ...
    def tissue_sub(self, input_fr, tissue_fr, mf_Qindices, scale_method = 'MSE', return_alg = 'one_tissue-fr' , area_minQ= 1.0, area_maxQ = 1.45, seek_mf = (-12,12,0.1), mf_min=1.0, mf_max=2.5 , window_size =4, show_result=False):
        ...
        if window_size!=None:
            window      = np.ones(window_size)/window_size                                    # window_size = 4 --> 0.25,0.25,0.25,0.225
            self.scaling_for = np.array([np.convolve(window, scaling_for[idx], 'same') for idx in range(scaling_for.shape[0])  ])             # outputs same length output and will be used for area and plotting
            self.scaling_by  = np.array([np.convolve(window, scaling_by[idx],  'same')  for idx in range(scaling_by.shape[0])   ])            # outputs same length output and will be used for area and plotting
